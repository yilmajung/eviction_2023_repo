...
head(df3$children_w_female_hh_ratio)
...
# Extract the results
posterior_estimates <- rstan::extract(fit_other)

stan_trace(fit_other, pars=c("beta_orig"))
stan_trace(fit_other, pars=c("beta"))
...
fit_other <- readRDS("data/results/fit_other_final5.rds")
...
df_90ci <- t(sapply(df_samples, function(x) quantile(x, probs = c(0.05, 0.95))))
df_95ci <- cbind(df_95ci, df_90ci)
write.csv(df_95ci, "data/results/df_95ci_other_final_5.csv")
View(df_95ci)
